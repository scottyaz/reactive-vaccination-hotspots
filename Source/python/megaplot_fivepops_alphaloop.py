## this is the script for running reactive vac similations for five
## populations
...
spotconfig = "HCCCC" # assuming we have one hotspot and 4 cold spots
...
for a in range(len(my_alphas)):
    ...
    for r in range(len_Rs):
        print ("\n R0 = %.2f, R1 = %.2f, R2 = %.2f, R3 = %.2f, R4 = %.2f, scenario %s of %s" %
               (Rs0[r],Rs1[r],Rs2[r],Rs3[r],Rs4[r],r,len_Rs))


        ins = vf.make_inputs(my_N,my_nlocs,
                             np.array([Rs0[r],Rs1[r],Rs2[r],Rs3[r],Rs4[r]]),my_alpha)
        ...
        for pv in range(len(pvacs)):
            ...
            vac_daily_target_nhs = np.concatenate(([0],
                                                   [total_vac],np.repeat([0],my_nlocs-2)))
            vac_daily_diffuse = np.array([total_vac/my_nlocs]*my_nlocs)

            ## now for targeting both hotspots or all coldsptos
            vac_daily_target_double = np.concatenate(([0],np.repeat(
                total_vac/(my_nlocs-1),my_nlocs-1)))
            ...
            for timing in range(len(timings_ref)):
                ...
                # now we are spreading it between cold spots
                tmp_run = spi.odeint(vf.SIR_dx_dt,
                                     ins[0],
                                     t=times,hmax=my_hmax,
                                     args=(ins[1],ins[2],ins[3],
